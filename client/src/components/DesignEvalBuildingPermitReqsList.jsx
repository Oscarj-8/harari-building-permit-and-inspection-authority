...
const DesignEvalBuildingPermitReqsList = () => {
  ...
  useEffect(() => {
    const fetchUserFolders = async () => {
      try {
        const response = await fetch("/api/user-designEval-folders");
  ...
  const handleDownloadFolder = (folderName) => {
    ...
    downloadLink.href = `/api/download-designEval/${folderName}`;
  ...
  const handleDeleteFolder = async () => {
    try {
      const response = await fetch(`/api/delete-designEval/${selectedFolder}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="flex flex-col p-1">
      <h1 className="text-lg underline">
        Design Evaluation and Building Permit Requests List
      </h1>
...
export default DesignEvalBuildingPermitReqsList;
